import { use, useEffect, useMemo, useRef } from 'react'
import { useFrame } from '@react-three/fiber'
import { useScroll } from '@react-three/drei'
import * as THREE from 'three'
import type { Feature } from 'geojson'
import { loadKoreaGeoJson } from '@/shared/api/koreaGeo'
import { cityVisualPoint } from '@/shared/lib/geo/cityProjection'

const CITY_TRANSITION_START = 0.2857
const FINAL_MAP_REVEAL_START = 0.9286

const CITIES = [
  { name: '서울', x: -10, z: -26 },
  { name: '인천', x: -16, z: -23 },
  { name: '대전', x: -1.4, z: 4 },
  { name: '대구', x: 22, z: 15 },
  { name: '부산', x: 31, z: 34 },
  { name: '광주', x: -13, z: 32 },
]

interface PulseRingProps {
  x: number
  z: number
  delay: number
  color: string
}

function PulseRing({ x, z, delay, color }: PulseRingProps) {
  const ref = useRef<THREE.Mesh>(null!)
  useFrame(({ clock }) => {
    if (!ref.current) return
    const t = ((clock.elapsedTime - delay) % 2.5) / 2.5
    ref.current.scale.setScalar(1 + t * 3)
    const mat = ref.current.material as THREE.MeshBasicMaterial
    mat.opacity = Math.max(0, 1 - t * 1.5)
  })
  return (
    <mesh ref={ref} position={[x, 0.1, z]} rotation={[-Math.PI / 2, 0, 0]}>
      <ringGeometry args={[0.3, 0.5, 32]} />
      <meshBasicMaterial color={color} transparent opacity={1} depthWrite={false} />
    </mesh>
  )
}

interface CityDotProps {
  x: number
  z: number
  color: string
  scale?: number
}

function CityDot({ x, z, color, scale = 1 }: CityDotProps) {
  return (
    <mesh position={[x, 0.2, z]}>
      <sphereGeometry args={[0.4 * scale, 8, 8]} />
      <meshBasicMaterial color={color} />
    </mesh>
  )
}

function lngLatToMapPoint(coord: number[], useCityScale = false) {
  const [lng, lat] = coord
  const x = (lng - 127.5) * 20
  const z = -(lat - 36.5) * 24
  if (!useCityScale) return new THREE.Vector3(x, 0.32, z)

  const point = cityVisualPoint(x, z)
  return new THREE.Vector3(point.x, 2.0, point.z)
}

function buildBoundaryGeometries(feature: Feature, useCityScale = false): THREE.BufferGeometry[] {
  const geom = feature.geometry as { type: string; coordinates: number[][][] | number[][][][] }
  const polygons: number[][][][] =
    geom.type === 'Polygon'
      ? [geom.coordinates as number[][][]]
      : (geom.coordinates as number[][][][])
  return polygons.map((polygon) => {
    const outer = polygon[0]
    const points = outer.map((coordinate) => lngLatToMapPoint(coordinate, useCityScale))
    if (points.length > 0) points.push(points[0].clone())
    return new THREE.BufferGeometry().setFromPoints(points)
  })
}

interface BoundaryLineProps {
  geometry: THREE.BufferGeometry
  color: string
  depthTest?: boolean
  depthWrite?: boolean
}

function BoundaryLine({
  geometry,
  color,
  depthTest = true,
  depthWrite = false,
}: BoundaryLineProps) {
  const line = useMemo(() => {
    const material = new THREE.LineBasicMaterial({
      color,
      transparent: true,
      opacity: 0.9,
      depthTest,
      depthWrite,
    })
    return new THREE.Line(geometry, material)
  }, [color, depthTest, depthWrite, geometry])

  useEffect(() => {
    return () => {
      ;(line.material as THREE.Material).dispose()
    }
  }, [line])

  return <primitive object={line} />
}

function GwangjuBoundary() {
  const scroll = useScroll()
  const mapRef = useRef<THREE.Group>(null!)
  const cityRef = useRef<THREE.Group>(null!)
  const geoJson = use(loadKoreaGeoJson())

  const feature = useMemo<Feature | null>(() => {
    return (
      geoJson.features.find(
        (item) => (item.properties as { name?: string } | null)?.name === '광주광역시'
      ) ?? null
    )
  }, [geoJson])

  const mapGeometries = useMemo(() => {
    if (!feature) return []
    return buildBoundaryGeometries(feature, false)
  }, [feature])

  const cityGeometries = useMemo(() => {
    if (!feature) return []
    return buildBoundaryGeometries(feature, true)
  }, [feature])

  useFrame(({ clock }) => {
    const t = scroll.offset
    const opacity = 0.82 + Math.sin(clock.elapsedTime * 3) * 0.12

    if (mapRef.current) {
      mapRef.current.visible = t >= 0.2143 && t < CITY_TRANSITION_START
      mapRef.current.children.forEach((line) => {
        const mat = (line as THREE.Line).material as THREE.LineBasicMaterial
        mat.opacity = opacity
      })
    }

    if (cityRef.current) {
      cityRef.current.visible = t >= FINAL_MAP_REVEAL_START
      cityRef.current.children.forEach((line) => {
        const mat = (line as THREE.Line).material as THREE.LineBasicMaterial
        mat.opacity = opacity
      })
    }
  })

  return (
    <>
      <group ref={mapRef} visible={false}>
        {mapGeometries.map((geometry, i) => (
          <BoundaryLine key={i} geometry={geometry} color="#ffffff" depthWrite={false} />
        ))}
      </group>
      <group ref={cityRef} visible={false}>
        {cityGeometries.map((geometry, i) => (
          <BoundaryLine
            key={i}
            geometry={geometry}
            color="#f2d58a"
            depthTest={false}
            depthWrite={false}
          />
        ))}
      </group>
    </>
  )
}

export default function MapMarkers() {
  const scroll = useScroll()
  const groupRef = useRef<THREE.Group>(null!)

  useFrame(() => {
    if (!groupRef.current) return
    const t = scroll.offset
    const visible = t < 0.2143 || (t >= 0.6429 && t < 0.7143)
    groupRef.current.visible = visible
  })

  return (
    <>
      <group ref={groupRef}>
        {[0, 0.6, 1.2].map((delay, i) => (
          <PulseRing key={i} x={-10} z={-26} delay={delay} color="#ff3333" />
        ))}
        <CityDot x={-10} z={-26} color="#ff4444" scale={1.2} />

        {CITIES.filter((c) => c.name !== '서울').map((city) => (
          <CityDot key={city.name} x={city.x} z={city.z} color="#ffffff" />
        ))}
      </group>
      <GwangjuBoundary />
    </>
  )
}
