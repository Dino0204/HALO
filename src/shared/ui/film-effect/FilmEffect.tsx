...
const SCENE_PARAMS: Array<[number, number, number, number]> = [
...
export function FilmEffect() {
  ...
  useFrame(({ clock }) => {
    ...
    const uniforms = effect.uniforms as Map<string, Uniform<number>>
    uniforms.get('uProgress')!.value = t
    uniforms.get('uTime')!.value = clock.elapsedTime
    ...
    uniforms.get('uGrain')!.value = prevParams.current.grain
    uniforms.get('uVignette')!.value = prevParams.current.vignette
    uniforms.get('uBW')!.value = prevParams.current.bw
    uniforms.get('uRedTint')!.value = prevParams.current.red
  })
