...
import { useSceneStore } from '@/entities/scene/model/sceneStore'
import { loadKoreaGeoJson } from '@/shared/api/koreaGeo'
import { GWANGJU_CITY_CENTER } from '@/shared/lib/geo/cityProjection'
import type { Point2D } from '@/shared/lib/geo/types'
import { GWANGJU_LANDMARKS } from '@/entities/gwangju/model/landmarks'
import { MBC_POS, CEMETERY_POS } from '@/entities/gwangju/model/landmarks'
...
const GWANGJU_MAP_FALLBACK: Point2D = { x: -13, z: 32 }
...
type CoordVisitor = (coord: number[]) => void
function visitCoordinates(coordinates: unknown, visitor: CoordVisitor): void {
  if (!Array.isArray(coordinates)) return
  if (typeof coordinates[0] === 'number') {
    visitor(coordinates as number[])
    return
  }
  coordinates.forEach((child) => visitCoordinates(child, visitor))
}

function getFeatureMapCenter(feature: Feature): Point2D {
  const bbox = {
    minLng: Infinity,
    minLat: Infinity,
    maxLng: -Infinity,
  ...
  visitCoordinates(geom.coordinates, ([lng, lat]: number[]) => {
...
function buildKeyframes(gwangjuMapCenter: Point2D): Keyframe[] {
  return [
...
function lerpKeyframes(t: number, keyframes: Keyframe[]): void {
  let i = 0
...
function applyStableCameraUp(camera: THREE.Camera, t: number): void {
...
export default function CameraRig() {
  ...
  const keyframesRef = useRef<Keyframe[]>(buildKeyframes(GWANGJU_MAP_FALLBACK))
